...
var shortestDayNames = moment.weekdaysShort()
var monthNames = moment.months()
var abbreviatedMonthNames = moment.monthsShort()
var amDesignator = 'AM'
var pmDesignator = 'PM'
...
function init(){
	 ...
	 abbreviatedDayNames = moment.weekdaysShort()
	 shortestDayNames = moment.weekdaysShort()
	 monthNames = moment.months()
	 abbreviatedMonthNames = moment.monthsShort()
	 amDesignator = 'AM'
	 pmDesignator = 'PM'
	 lowerAMDesignator = 'am'
	 lowerPMDesignator = 'pm'

	 regexDayNames = new RegExp(dayNames.join('|'), 'i')
	 regexAbbreviatedDayNames = new RegExp(abbreviatedDayNames.join('|'), 'i')
	 regexShortestDayNames = new RegExp('\\b(' + shortestDayNames.join('|') + ')\\b', 'i')
	 regexMonthNames = new RegExp(monthNames.join('|'), 'i')
	 regexAbbreviatedMonthNames = new RegExp(abbreviatedMonthNames.join('|'), 'i')

	 regexFirstSecondThirdFourth = /(\d+)(st|nd|rd|th)\b/i
	 regexEndian = /(\d{1,4})([\/\.\-])(\d{1,2})[\/\.\-](\d{1,4})/

	 regexTimezone = /((\+|\-)\d\d:\d\d)$/
     regexTimezone2 = /((\+|\-)\d\d)$/
     
	 amOrPm = '(' + [amDesignator, pmDesignator].join('|') + ')'
	 lowerAmOrPm = '(' + [lowerAMDesignator, lowerPMDesignator].join('|') + ')'
	 regexLowerAmOrPm = new RegExp(lowerAmOrPm)
	 regexUpperAmOrPm = new RegExp(amOrPm)
	 regexHoursWithLeadingZeroDigitMinutesSecondsAmPm = new RegExp('0\\d\\:\\d{1,2}\\:\\d{1,2}(\\s*)' + amOrPm, 'i')
	 regexHoursWithLeadingZeroDigitMinutesAmPm = new RegExp('0\\d\\:\\d{1,2}(\\s*)' + amOrPm, 'i')
	 regexHoursWithLeadingZeroDigitAmPm = new RegExp('0\\d(\\s*)' + amOrPm, 'i')
	 regexHoursMinutesSecondsAmPm = new RegExp('\\d{1,2}\\:\\d{1,2}\\:\\d{1,2}(\\s*)' + amOrPm, 'i')
	 regexHoursMinutesAmPm = new RegExp('\\d{1,2}\\:\\d{1,2}(\\s*)' + amOrPm, 'i')
	 regexHoursAmPm = new RegExp('\\d{1,2}(\\s*)' + amOrPm, 'i')

	 regexISO8601HoursWithLeadingZeroMinutesSecondsMilliseconds = /\d{2}:\d{2}:\d{2}\.\d{3}/
	 regexISO8601HoursWithLeadingZeroMinutesSecondsCentiSeconds = /\d{2}:\d{2}:\d{2}\.\d{2}/
	 regexISO8601HoursWithLeadingZeroMinutesSecondsDeciSeconds = /\d{2}:\d{2}:\d{2}\.\d{1}/
	 regexHoursWithLeadingZeroMinutesSeconds = /0\d:\d{2}:\d{2}/
	 regexHoursWithLeadingZeroMinutes = /0\d:\d{2}/
	 regexHoursMinutesSeconds = /\d{1,2}:\d{2}:\d{2}/
	 regexHoursMinutes = /\d{1,2}:\d{2}/
	 regexYearLong = /\d{4}/
	 regexDayLeadingZero = /0\d/
	 regexDay = /\d{1,2}/
	 regexYearShort = /\d{2}/     

	 regexDayShortMonthShort = /^([1-9])\/([1-9]|0[1-9])$/
	 regexDayShortMonth = /^([1-9])\/(1[012])$/
	 regexDayMonthShort = /^(0[1-9]|[12][0-9]|3[01])\/([1-9])$/
	 regexDayMonth = /^(0[1-9]|[12][0-9]|3[01])\/(1[012]|0[1-9])$/

	 regexMonthShortYearShort = /^([1-9])\/([1-9][0-9])$/
	 regexMonthYearShort = /^(0[1-9]|1[012])\/([1-9][0-9])$/

	 formatIncludesMonth = /([\/][M]|[M][\/]|[MM]|[MMMM])/

	 regexFillingWords = /\b(at)\b/i

	 regexUnixMillisecondTimestamp = /\d{13}/
...
function parseFormat (dateString, options) {
  ...
  format = format.replace(regexTimezone2, 'Z')
  ...
  //format = format.replace(regexUTC, 'Z')
  
  // now, the next number, if existing, must be a day, or UTC
